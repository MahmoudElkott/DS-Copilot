...
class PipelineSession(Base):
    ...
    state_dump = Column(JSON, nullable=False)
...
class MemoryManager:
    ...
    def save_pipeline_state(self, session_id: str, state: InternalPipelineModel):
        session = self.Session()
        try:
            pipeline_session = session.query(PipelineSession).filter_by(session_id=session_id).first()
            if not pipeline_session:
                pipeline_session = PipelineSession(session_id=session_id, state_dump=state.model_dump(mode="json"))
                session.add(pipeline_session)
            else:
                pipeline_session.state_dump = state.model_dump(mode="json")
            session.commit()
        finally:
            session.close()

    def get_pipeline_state(self, session_id: str) -> Optional[InternalPipelineModel]:
        session = self.Session()
        try:
            pipeline_session = session.query(PipelineSession).filter_by(session_id=session_id).first()
            if pipeline_session and pipeline_session.state_dump:
                return InternalPipelineModel.model_validate(pipeline_session.state_dump)
            return None
        finally:
            session.close()
