...
from app.infrastructure.executor import BaseExecutor, ExecutionResult
...
class HarnessEngine:
    ...
    def _get_model(self, task_type: str):
        if not self.llm_router:
            return None
        return self.llm_router.get_model(
            task_type=task_type,
            provider_override=self.runtime_settings.get("llm_provider"),
            model_override=self.runtime_settings.get("model_name"),
...
"""
        if self.llm_router:
            model = self._get_model(task_type="testing")
    ...
    async def _auto_fix(
        ...
        code_for_prompt = truncate(code, max_len=12000)
        error_for_prompt = truncate(error, max_len=3000)
        truncation_note = ""
        if len(code_for_prompt) < len(code):
            truncation_note += "\n- Original code was truncated for context safety."
        if len(error_for_prompt) < len(error):
            truncation_note += "\n- Full error log was truncated for context safety."
...
{truncation_note}
"""
        model = self._get_model(task_type="code_writing")
        from langchain_core.messages import HumanMessage
